...
class App extends Component{
  ...
  async getWeather(){
    try{
      ...
      if(!geoData.results) throw new Error("Data not found");
  ...
  render(){
    ...
    return(
      <div className="container">
        <h1>Weather App</h1>
        <input type="text" placeholder="Search Location Based Weather" value={this.state.location} onChange={(e)=> this.setState({location: e.target.value})}/>
...
class Weather extends Component{
  render(){
    const {temperature_2m_max: max, 
      temperature_2m_min: min, time, weathercode
    } = this.props.weather;
    ...
    return(
      <div>
        ...
        <ul className="weather">
          {
            time.map((date, i)=>(
              <Day date={date} minTime={min.at(i)} maxTime={max.at(i)} code={weathercode.at(i)} key={date} />
            ))}
        </ul>
      </div>
    );
  ...
  render(){
    ...
    return(
      <div className="day">
        <span>{code}</span>
        <p>{date}</p>
        <p>{Math.floor(minTime)} &deg; &mdash; {Math.ceil(maxTime)} &deg;</p>
      </div>
